...
export default class GenerateUserWallet extends BaseCommand {
  ...
  async run() {
    this.logger.info(`Starting generation of wallets for user id: ${this.userId}`);
    ...
    this.logger.info(`EOA address: ${result["externallyOwnedAccount"]?.publicAddress}`);
    this.logger.info(`SCA address: ${result["smartContractAccount"]?.publicAddress}`);
  }
}
