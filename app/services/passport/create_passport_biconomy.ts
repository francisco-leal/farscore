...
import { generateWalletsBiconomy } from "#services/generate_wallets_biconomy";

import { User } from "#models/user";
import { Passport } from "#models/passport";

export const createPassportBiconomy = async (userId: number) => {
	...
	const walletsResult = await generateWalletsBiconomy(userId);
	...
	const smartAccount = await createSmartAccountClient({
		signer,
		paymaster: new BiconomyPaymaster({ paymasterUrl: config.paymasterUrl }),
